...
### Item fit
```{r}
simfit <- RIgetfit(df, iterations = 1000, cpu = 4) # save simulation output into object `simfit` (arbitrary name)
RIitemfit(df, simcut = simfit)
```
### CICC
```{r}
...
```{r}
res_cor <- RIgetResidCor(df, iterations = 1000, cpu = 8)
RIresidcorr(df, cutoff = res_cor$p99)
```
